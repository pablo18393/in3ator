    ...
    if (digitalRead(pulse) == 0) {
      ...
      if (!(page == 3 || page == 4 || page == 6)) {
        for (int i = 2; i <= rectangles; i++) {
          tft.fillRect(0, (240 - width_heading) * (i - 1) / rectangles + width_heading - 1, 240, width_space, WHITE); //mejorable
        }
      }
      else {
...
void eraseBigBar() {
  tft.fillRect(0, width_heading, width_select, (240 - width_heading) * 3 / rectangles, COLOR_BAR);
}
